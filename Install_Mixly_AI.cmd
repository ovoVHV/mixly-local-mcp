...
rem One-click installer for Mixly 2/3/4 and the DeepSeek Harness client.
cd /d "%~dp0"
...
echo Mixly AI installer for Mixly 2 / 3 / 4
echo Package directory: %MCP_SOURCE%
echo Leave a version path empty to skip that Mixly generation.
echo.
...
if not defined MIXLY4_HOME if not defined MIXLY2_HOME if not defined MIXLY3_HOME (
  echo At least one Mixly path is required.
  pause
  exit /b 2
)
...
if defined MIXLY4_HOME (
  ...
  set MIXLY4_ARG=--mixly4-home "%MIXLY4_HOME%"
)
...
node "%MCP_SOURCE%\harness_integration\install.js" --mcp-source "%MCP_SOURCE%" %MIXLY4_ARG% %MIXLY2_ARG% %MIXLY3_ARG%
if errorlevel 1 (
...
echo.
echo Installation complete for the selected Mixly generations.
echo Close and reopen each selected Mixly application once so its AI toolbar button is loaded.
echo Then click the sparkle button in the top toolbar.
pause
exit /b 0
