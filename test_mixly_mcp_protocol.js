...
async function main() {
  ...
  assert.equal(initialized.result.serverInfo.version, '2.2.0');
  ...
  const specs = await request('tools/call', {
    ...
    arguments: {
      ...
      blockTypes: ['DHT', 'display_rgb', 'controls_if', 'variables_declare']
    }
  });
  ...
  const createdLibrary = await request('tools/call', {
    ...
    arguments: {
      ...
      board: 'default/arduino_avr',
      overwrite: true,
      blocksJs: "Blockly.Blocks['mcp_test_primitive']={init:function(){this.appendDummyInput().appendField('测试底层原语');this.setPreviousStatement(true);this.setNextStatement(true);}};",
      generatorsJs: "Blockly.Arduino.forBlock['mcp_test_primitive']=function(){return 'delay(1);\\n';};",
